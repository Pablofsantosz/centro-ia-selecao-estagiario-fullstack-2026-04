  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResult("Erro ao gerar testes: " + (data.detail || "Erro "));
      }
    } catch (error) {
      setResult("Erro na conexão com api");
    } finally {
      setLoading(false); 
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <form onSubmit={handleGenerate} className="space-y-4 bg-white p-6 rounded-lg shadow-md border border-gray-200">
          ...
          <textarea
            ...
            rows="4"
            placeholder="Ex: Como utilizador, quero poder redefinir a minha palavra-passe clicando em 'Esqueci-me da palavra-passe'."
            value={description}
          ...
          >
            {loading ? "A gerar Testes com IA (Aguarde)" : "Gerar Cenários e Automação"}
          </button>
        </form>
